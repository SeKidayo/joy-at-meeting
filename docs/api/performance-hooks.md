...
- `deps` (array): 依赖数组
- `compare` (function, 可选): 自定义比较函数

### 返回值

- `memoizedValue` (T): 缓存的计算结果

### 复杂计算缓存

```tsx
import { useMemo } from 'joy-at-meeting'

function ExpensiveCalculation({ data }: { data: number[] }) {
  // 缓存复杂计算结果
  const statistics = useMemo(() => {
    console.log('计算统计信息...') // 只在 data 变化时执行
  ...
  return (
    <div>
      ...
      <p>平均值: {statistics.avg}</p>
      <p>最大值: {statistics.max}</p>
      <p>最小值: {statistics.min}</p>
      <p>方差: {statistics.variance}</p>
      <p>标准差: {statistics.stdDev}</p>
    </div>
  )
}
```
...
```tsx
function UserProfile({ user }: { user: { id: string, profile: any } }) {
  // 使用深度比较，只有当 profile 内容真正变化时才重新计算
  const formattedProfile = useMemo(
    () => {
      console.log('格式化用户资料...') // 只在 profile 内容变化时执行
      ...
      return {
        ...
        bio: user.profile.bio || '这个人很懒，什么都没写',
  ...
  return (
    <div style={{ display: 'flex', gap: '1rem', padding: '1rem' }}>
      <img 
        src={formattedProfile.avatar} 
        alt="头像" 
        style={{ width: '100px', height: '100px', borderRadius: '50%' }}
      />
      <div>
        <h2>{formattedProfile.displayName}</h2>
        <p>年龄: {formattedProfile.age}</p>
        <p>位置: {formattedProfile.location}</p>
        <p>简介: {formattedProfile.bio}</p>
      </div>
    </div>
  )
}
